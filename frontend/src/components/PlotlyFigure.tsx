...
export function PlotlyFigure({ figureJson, className, height, onReady, onClick }: PlotlyFigureProps) {
  ...
  useEffect(() => {
    ...
    const config = { displaylogo: false, displayModeBar: false, responsive: true, ...(parsed.config ?? {}) };
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    if (onClick && plotDiv) (plotDiv as any).on("plotly_click", onClick);

    const handleResize = () => { if (ref.current) Plotly.Plots.resize(ref.current); };
    window.addEventListener("resize", handleResize);
    return () => { window.removeEventListener("resize", handleResize); if (ref.current) Plotly.purge(ref.current); };
