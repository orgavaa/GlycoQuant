      ...
      {result.glyco_mechano_correlation_figure_json && (
        <>
          <div>
            ...
            <PlotlyDark figureJson={result.glyco_mechano_correlation_figure_json} height={240} />
          </div>
          <Sep />
        </>
      )}
      ...
      {result.mechano_score_distribution_figure_json && (
        <>
          <div>
            ...
            <PlotlyDark figureJson={result.mechano_score_distribution_figure_json} height={120} />
          </div>
          <Sep />
        </>
      )}
      ...
      {topCells.length > 0 && (
        <div>
          ...
          {topCells.map(cell => {
            ...
            const isPos = ms >= 0;
            return (
              <div key={cell.cell_id} onClick={() => setSelectedCellId(Number(cell.cell_id))} style={{
                display: "flex", alignItems: "center", padding: "8px 0",
                borderBottom: "1px solid #1a1a1a", cursor: "pointer", fontSize: 12,
              }}>
                <span style={{ color: "#666", width: 36, flexShrink: 0 }}>#{cell.cell_id}</span>
...
function HeroMetric({ value, label }: { value: string; label: string }) {
  return (
    <div style={{ flex: 1, textAlign: "center" }}>
      <div style={{
        fontSize: 26, fontWeight: 700, color: value === "\u2014" ? "#444" : "#eee", lineHeight: 1,
        fontVariantNumeric: "tabular-nums",
      }}>{value}</div>
      <div style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: 1, color: "#666", marginTop: 6, fontWeight: 500 }}>{label}</div>
    </div>
  );
}
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return <div style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: 1.2, color: "#666", marginBottom: 10, fontWeight: 600 }}>{children}</div>;
}
